          ...
          while (1) {
            switch (_context.prev = _context.next) {
              ...
              case 2:
                ...
                return _context.abrupt('return', normalizeRoutes(routes, { disableDuplicateRoutesWarning: disableDuplicateRoutesWarning }));
...
function normalizeRoutes(routes) {
  ...
  var _force404$config = _extends({ force404: true }, config),
      tree = _force404$config.tree,
      disableDuplicateRoutesWarning = _force404$config.disableDuplicateRoutesWarning,
      force404 = _force404$config.force404;
  ...
  var recurse = function recurse(route) {
    ...
    var originalPath = route.is404 ? path404 : (0, _shared.pathJoin)(route.path);
    ...
    if (normalizedRoute.children) {
      normalizedRoute.children = route.children.map(function (d) {
    ...
    var found = flatRoutes.find(function (d) {
      return d.path === normalizedRoute.path;
    });
  ...
  var normalizedRoutes = routes.map(function (d) {
    return recurse(d);
  });

  if (force404 && !flatRoutes.find(function (d) {
    return d.is404;
  })) {
    flatRoutes.push({
      is404: true,
